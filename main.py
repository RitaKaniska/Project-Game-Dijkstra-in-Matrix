...
class myGUI():
    def __init__(self,app):
        ...
        app.title("TÌM ĐƯỜNG ĐI NGẮN NHẤT")
        # setting window size
        width=810
        height=720
        ...
        but_map = tkinter.Button(top_frame,text="TẠO MÊ CUNG",font=tkinter.font.Font(size=20),command=self.draw_graph, bg = '#3F645A')
        but_run = tkinter.Button(top_frame,text="CHẠY",font=tkinter.font.Font(size=20),command=self.VisibleMode, bg = '#f9ee9c')
        but_dji = tkinter.Button(top_frame,text="DIJKSTRA",font=tkinter.font.Font(size=20),command=self.dijkstra, bg = '#FFA69E')
        
        but_vis = tkinter.Button(top_frame,text="ẨN/HIỆN",font=tkinter.font.Font(size=20),command=self.checkVisible,bg = '#5E6472')
        
        self.visible = False

        self.canvas = tkinter.Canvas(bot_frame,width=840,height=600,background='#A7C4C2')
        ...
        self.infoText = tkinter.Label(bot_frame, font=tkinter.font.Font(size=14),text="")
        ...
        self.graph = {}
        self.vertices = {}
        self.edges = [[-1 for i in range(ncolumn)] for j in range(nrow)]

    def UpdateInfo(self):
        text = ""
        if self.checkStart:
            text = "Điểm khởi đầu: [" + ("%.2f" % (maze.points[0][0]+1)) + (", %.2f" % (nrow - 1 - maze.points[0][1])) + "]  |  "
        if self.checkEnd:
            text += "Điểm kết thúc: [" + ("%.2f" % (maze.points[1][0]+1)) + (", %.2f" % (nrow - 1 - maze.points[1][1])) + "]\n"
        ...
        if self.dijkstra_time != -1 and self.graph_time != -1: 
            text += "  Tổng thời gian: " + ("%.5f" % (self.dijkstra_time + self.graph_time)) + "s\n"
        if self.distance != -1:
    ...
    def gen_map(self):
        ...
        for i in range(len(maze.points)):
            self.graph[i]=[]
            self.vertices[i]=[]
        self.canvas.create_polygon(maze.pointsW[2:len(maze.pointsW)],fill = "#3A9F19")
        self.visible = False
    
    def draw_graph(self, redraw = True):
        self.canvas.delete("all")
        if redraw: self.gen_map()
        else: self.canvas.create_polygon(maze.pointsW[2:len(maze.pointsW)],fill = "#3A9F19")
        if (self.visible):
        ...
        if (self.checkEnd):
            self.draw_point(maze.points[1][0] + 1, maze.points[1][1] + 1, "#191D9F", "E")
    ...
    def dijkstra(self):
        ...
        if (self.checkStart and self.checkEnd and self.checkGraph and self.SinP and self.EinP):
            ...
            print(self.res)
            self.UpdateInfo()
        else: tkinter.messagebox.showinfo('Response', "Please type in two valid points!")

    def VisibleMode(self, event = 0): # mode 0 - graph w/o starting and ending point, mode 1 - update starting/ending point
        s,e = 0,0
        timeStart = time.time()
        #self.visible = True
        l = len(maze.points)
        ...
        if (event == 0):
            ...
            if (self.checkStart or self.checkEnd): 
                self.VisibleMode(1)
                return
        #if (event == 2 and self.linkSE == False and self.checkStart): 
            #self.VisibleMode(1)
        #print(self.graph[0])
